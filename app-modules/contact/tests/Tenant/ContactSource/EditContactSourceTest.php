...
use AidingApp\Contact\Tests\Tenant\ContactSource\RequestFactories\EditContactSourceRequestFactory;
use App\Models\User;
...
test('EditContactSource requires valid data', function (EditContactSourceRequestFactory $data, array $errors) {
    asSuperAdmin();
    ...
    [
        'name missing' => [fn () => EditContactSourceRequestFactory::new()->state(['name' => null]), ['name' => 'required']],
        'name not a string' => [fn () => EditContactSourceRequestFactory::new()->state(['name' => 1]), ['name' => 'string']],
    ]
);
