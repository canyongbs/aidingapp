...
it('validates request', function (array $data, array $expectedErrors) {
    ...
    actingAs($user)
        ->postJson(route('in-app-communication.conversations.messages.store', $conversation), $data)
        ->assertStatus(422)
    ...
    'content must be array' => [
        ['content' => 'not-array'],
        ['content' => 'The content must be an array.'],
    ],
]);
...
it('requires participant to send messages', function () {
    ...
    actingAs($user)
        ->postJson(route('in-app-communication.conversations.messages.store', $conversation), [
