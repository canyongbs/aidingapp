...
it('returns public channels the user is not a member of', function () {
    ...
    $privateChannel = Conversation::factory()->channel()->create([
        'name' => 'Private Channel',
        'is_private' => true,
    ]);

    $memberChannel = Conversation::factory()->channel()->create([
        'name' => 'Member Channel',
        'is_private' => false,
    ]);
    ConversationParticipant::factory()->create([
        'conversation_id' => $memberChannel->getKey(),
        'participant_id' => $user->getKey(),
    ]);

    actingAs($user)
        ->getJson(route('in-app-communication.conversations.public'))
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.name', 'Public Channel');
});
