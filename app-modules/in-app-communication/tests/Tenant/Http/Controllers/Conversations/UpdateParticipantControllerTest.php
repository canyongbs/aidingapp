...
it('validates request', function (array $data, array $expectedErrors) {
    ...
    actingAs($user)
        ->patchJson(route('in-app-communication.conversations.participants.update', [$conversation, $participant]), $data)
        ->assertStatus(422)
        ->assertJsonValidationErrors($expectedErrors);
})->with([
    'is_manager is required' => [
        [],
        ['is_manager' => 'The is manager field is required.'],
    ],
    'is_manager must be boolean' => [
        ['is_manager' => 'not-boolean'],
