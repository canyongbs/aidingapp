...
namespace AidingApp\Project\Filament\Concerns;
...
trait HasPipelineSwitcherAction
{
    public function selectPipelineAction(): Action
    {
        return Action::make('selectPipeline')
            ...
            ->fillForm(fn (): array => ['pipeline_id' => $this->getPipelineSwitcherCurrentPipelineId()])
            ->schema([
                TableSelect::make('pipeline_id')
                    ->hiddenLabel()
                    ->tableConfiguration(ProjectPipelinesTable::class)
                    ->tableArguments(['projectId' => $this->getPipelineSwitcherProjectId()])
                    ->required(),
            ])
