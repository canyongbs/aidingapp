...
test('ListAdvisorySeverities is gated with proper access control', function () {
    $user = User::factory()->create();

    actingAs($user)
        ->get(
            AdvisorySeverityResource::getUrl('index')
        )->assertForbidden();
...
test('it is gated with proper feature access control', function () {
    ...
    get(ListAdvisorySeverities::getUrl())->assertForbidden();
    ...
    get(ListAdvisorySeverities::getUrl())->assertSuccessful();
});
...
test('can list records', function () {
    ...
    $records = AdvisorySeverity::factory()->count(5)->create();
...
test('bulk delete AdvisorySeverities', function () {
    $user = User::factory()->create();
    ...
    $advisorySeverities = AdvisorySeverity::factory()->count(10)->create();

    livewire(ListAdvisorySeverities::class)
        ->callTableBulkAction(DeleteBulkAction::class, $advisorySeverities);

    foreach ($advisorySeverities as $advisorySeverity) {
        assertSoftDeleted($advisorySeverity);
    }
});

test('prevent deletion of AdvisorySeverity if it has associated Advisories', function () {
    $user = User::factory()->create();
    ...
    $advisorySeverities = AdvisorySeverity::factory()
        ->has(Advisory::factory(), 'advisories')
        ->count(2)
        ->create();
