...
test('ListAdvisoryStatuses is gated with proper access control', function () {
    $user = User::factory()->create();

    actingAs($user)
        ->get(
            AdvisoryStatusResource::getUrl('index')
        )->assertForbidden();
...
test('it is gated with proper feature access control', function () {
    ...
    get(ListAdvisoryStatuses::getUrl())->assertForbidden();
    ...
    get(ListAdvisoryStatuses::getUrl())->assertSuccessful();
});
...
test('can list records', function () {
    ...
    $records = AdvisoryStatus::factory()->count(5)->create();
...
test('bulk delete AdvisoryStatuses', function () {
    $user = User::factory()->create();
    ...
    $advisoryStatuses = AdvisoryStatus::factory()->count(10)->create();

    livewire(ListAdvisoryStatuses::class)
        ->callTableBulkAction(DeleteBulkAction::class, $advisoryStatuses);

    foreach ($advisoryStatuses as $advisoryStatus) {
        assertSoftDeleted($advisoryStatus);
    }
});

test('prevent deletion of AdvisoryStatus if it has associated Advisories', function () {
    $user = User::factory()->create();
    ...
    $advisoryStatuses = AdvisoryStatus::factory()
