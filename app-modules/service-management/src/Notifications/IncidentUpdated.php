...
class IncidentUpdated extends BaseNotification implements EmailNotification, DatabaseNotification
{
    ...
    public function toEmail(object $notifiable): MailMessage
    {
        return MailMessage::make()
            ...
            ->subject("New update on incident {$this->incident->title}")
            ->line("The incident {$this->incident->title} has a new update.")
    ...
    public function toDatabase(object $notifiable): array
    {
        return Notification::make()
            ->success()
            ->title((string) str("New update on [incident {$this->incident->title}](" . IncidentResource::getUrl('view', ['record' => $this->incident]) . ')')->markdown())
            ->getDatabaseMessage();
    }

    private function resolveNotificationSetting(object $notifiable): ?NotificationSetting
    {
        return $this->incident->division?->notificationSetting?->setting; //check on this
    }
}
