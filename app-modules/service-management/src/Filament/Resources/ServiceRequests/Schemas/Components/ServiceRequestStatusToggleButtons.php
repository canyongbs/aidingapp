...
class ServiceRequestStatusToggleButtons
{
    public static function make(string $name = 'status_id'): ToggleButtons
    {
        return ToggleButtons::make($name)
            ->label('Status')
            ->inline()
            ->options(fn () => ServiceRequestStatus::orderBy('sort')->pluck('name', 'id'))
            ->colors(fn () => ServiceRequestStatus::orderBy('sort')
                ->get(['id', 'color'])
                ->mapWithKeys(fn (ServiceRequestStatus $status): array => [$status->getKey() => $status->color->value]))
