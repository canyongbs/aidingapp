...
class EditServiceRequestContactAction
{
    public static function make(ServiceRequest $serviceRequest): Action
    {
        return Action::make('editContact')
            ...
            ->slideOver()
            ->modalHeading('Edit Customer Contact')
            ->modalSubmitActionLabel('Save')
            ->fillForm([
                'respondent_id' => $serviceRequest->respondent_id,
            ])
            ->schema([
                TableSelect::make('respondent_id')
                    ->label('Customer Contact')
                    ->tableConfiguration(ServiceRequestContactsTable::class)
                    ->exists((new Contact())->getTable(), 'id')
                    ->required(),
            ])
            ->action(function (array $data) use ($serviceRequest): void {
                $serviceRequest->respondent_id = $data['respondent_id'];
                $serviceRequest->unsetRelation('respondent');
                $serviceRequest->save();
