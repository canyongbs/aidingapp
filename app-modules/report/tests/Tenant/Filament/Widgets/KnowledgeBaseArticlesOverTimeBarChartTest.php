...
it('returns correct articles over time data without filters', function () {
    ...
    $widget->cacheTag = 'test-kb-articles-over-time';
    $widget->pageFilters = [];

    $data = $widget->getData();

    expect($data['labels'])->toHaveCount(12)
        ->and(array_sum($data['datasets'][0]['data']))->toEqual(5);
});

it('filters articles over time by category', function () {
    $status = KnowledgeBaseStatus::factory()->create();
    ...
    $widget = new KnowledgeBaseArticlesOverTimeBarChart();
    $widget->cacheTag = 'test-kb-articles-over-time-filtered';
    $widget->pageFilters = [
    ...
    expect(array_sum($data['datasets'][0]['data']))->toEqual(2);
});
