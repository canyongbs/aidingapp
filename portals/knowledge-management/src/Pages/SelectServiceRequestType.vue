...
<script setup>
    ...
    // The available categories and types arrive via the route data loader.
    const { data: initialData } = useServiceRequestTypesData();
    ...
    function syncCurrentCategory() {
        ...
        if (categoryId && categories.value.length > 0) {
            currentCategory.value = findById(categories.value, categoryId) || null;
        } else {
    ...
    watch(() => route.params.categoryId, syncCurrentCategory);
