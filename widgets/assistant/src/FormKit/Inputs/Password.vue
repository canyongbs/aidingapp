...
<script setup>
    ...
    function storeSecret(shouldRemove = false) {
        ...
        if (storagePromise.value) return storagePromise.value;
        if (!isDirty.value) {
        ...
        storagePromise.value = (async () => {
            try {
                ...
                typedValue.value = '';
                storedSecretId.value = response.data.id;
                isEditing.value = response.data.id === null;
                isDirty.value = false;
                await props.context.node.input(response.data.id);

                return true;
            } catch (error) {
                if (!error.response || error.response.status === 401) clearToken();
