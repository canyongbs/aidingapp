...
The scheduler is a **flat, landlord-level** schedule. It does **not** loop over tenants and does **not** use `tenants:artisan`. Per-tenant recurring work must be a **fan-out orchestrator** extending `App\Jobs\DispatchForEachTenant`, scheduled once with `->onOneServer()`; the orchestrator enqueues one child job per eligible tenant. Almost every entry uses `->onOneServer()` — the one deliberate exception is the schedule liveness beacon (see the third trap).
...
- **The schedule liveness beacon must NOT use `onOneServer`:** the final `$schedule->call(... touch(storage_path('framework/schedule-heartbeat')) ...)->name('Schedule Liveness Beacon')` entry is deliberately per-node. Each running scheduler task's container healthcheck reads its own heartbeat file, so every node must touch it every minute; `onOneServer` would leave the other nodes' files stale and fail their healthchecks. Do not add `onOneServer` to it.
